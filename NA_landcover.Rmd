...
gdd5 <- raster('Data/gdd5_global.tif') %>% crop(bbox)
...
sample_guide <- dat %>% group_by(pft) %>% count %>% mutate(n  = ifelse(n > 300000, 30000, n))
...
mod <- gam(list(pft_class
                 ...
                 ~ s(alpha, bs = 'cr') + s(gdd5, bs = 'cr') + s(t_cold, bs = 'cr') + s(t_warm, bs = 'cr'),
                 ~ s(alpha, bs = 'cr') + s(gdd5, bs = 'cr') + s(t_cold, bs = 'cr') + s(t_warm, bs = 'cr')), 
           family = multinom(K = 6), method = 'REML', select = F, data = dat2, optimizer = 'perf')

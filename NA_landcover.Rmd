...
plot(brick(c(ad1950, sum(prec[[c(12, 1, 2)]]))))

brick(c(ad200 - ad1950, ad850 - ad1950, ad200 - ad850)) %>% levelplot
...
lm.pet <- hargreaves(lm.tmin %>% getValues %>% t, 
                  lm.tmax %>% getValues %>% t, 
                  Ra = srad %>% getValues %>% t, 
                  Pre = lm.prec %>% getValues %>% t, na.rm = T) %>%
  t %>%
  setValues(lm.tmin, .)
...
for(a in 1:10){
  for(i in 1:12){
    ...
    aet.brick[[i]] <- lm.aet
    swc.tmp <- swc + lm.prec[[i]] * (1 - .15) - lm.aet
    swc <- reclassify(swc.tmp, c(-Inf, 0, 0, 350, Inf, 350))
  }
}

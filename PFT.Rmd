...
test<- dat2 %>% filter(megabiome == 'temperate forest' & gdd5 <= 900 & gdd5 >= 350)
test <- dat2 %>% filter((megabiome == 'temperate forest' | megabiome == 'boreal forest') & gdd5 < 350)
...
ggplot(test, aes(x = megabiome, fill = megabiome)) +
  geom_bar()
...
levelplot(gdd5)
biome.preds <- which.max(predictions)# %>% ratify
rasterVis::gplot(biome.preds) + geom_tile(aes(fill = as.factor(value))) + coord_fixed()
rasterVis::gplot(predictions[[1]]) + geom_tile(aes(fill = value)) + coord_fixed()

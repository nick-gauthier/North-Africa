...
prec.mod4 <- bam(prec.obs ~ s(precl, bs = 'cr', k = 10) +
                    te(precc, month, bs = c('cr', 'cc'), k = c(10, 5)) +
                   s(elev, bs = 'cr', k = 10) +
                    s(vel, bs = 'cr', k = 10) +
                    te(rough, dco),
                data = prec.dat, family = Gamma(link = 'log'), knots = list(month = c(0.5, seq(1, 12, length = 3), 12.5)), select = T)
...
```{r}
dat %>% filter(prec.obs > 0) %>%
ggplot(aes(elev, prec.mod3$residuals)) +
  geom_point(aes(color = as.factor(month)), alpha = .1, size = .3) +
  geom_smooth(aes(color = as.factor(month))) +
  theme_minimal()
```

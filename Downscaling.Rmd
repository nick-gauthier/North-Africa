...
myenv <- rsaga.env(modules='/usr/lib/x86_64-linux-gnu/saga/') 
#output the DEM raster into a SAGA readable format 

dir.to <- (90 - atan2(v, u) * (180 / pi)) %% 360

writeRaster(elev, "elev.sgrd", format="SAGA", overwrite=TRUE) 
flow.height <- projectRaster(u, elev)
wind.effect <- projectRaster(u, elev)
for(i in 1:12){
  writeRaster(projectRaster(dir.to[[i]], elev), "dir.sgrd", format="SAGA", overwrite=TRUE) 
  writeRaster(projectRaster(vel[[i]], elev), "vel.sgrd", format="SAGA", overwrite=TRUE)
  rsaga.geoprocessor("ta_morphometry", module = 15, param = list(DEM="elev.sgrd",DIR="dir.sgrd",DIR_UNITS='1',LEN="vel.sgrd",LEN_SCALE=1, EFFECT="oro", AFH='flowheight', MAXDIST=300.0),env=myenv) 
  flow.height[[i]] <- raster('flowheight.sgrd', native = T)
  wind.effect[[i]] <- raster('oro.sgrd', native = T)
}
 
wind.effect <- wind.effect %>% mask(na.mask)
flow.height <- flow.height %>% mask(na.mask)
levelplot(log(flow.height))
...
vars <- sapply(1:12, function(x){
  brick(temp.obs[[x]], prec.obs[[x]], trefht[[x]], precc[[x]], precl[[x]], dir[[x]] %>% projectRaster(elev), 
        vel[[x]] %>% projectRaster(elev), delta[[x]] %>% projectRaster(elev), oro[[x]], lift.angle[[x]], aco[[x]], dco, rough, elev, slope%>% projectRaster(elev), aspect %>% projectRaster(elev), lat, lon, month[[x]], temp.pred[[x]]) %>%
    setNames(c('temp.obs', 'prec.obs', 'trefht', 'precc', 'precl', 'dir', 'vel', 'delta', 'oro', 'lift.angle', 'aco', 'dco', 'rough', 'elev', 'slope', 'aspect', 'lat', 'lon', 'month', 'temp'))
})
...
Looks safe to use a single precl. What's going on whith precc?
```{r}
dat %>% filter(prec.obs > 0) %>%
...
prec.mod1$aic;prec.mod2$aic;prec.mod3$aic;prec.mod4$aic

plot(prec.mod4, pers = T, pages = 1)
```

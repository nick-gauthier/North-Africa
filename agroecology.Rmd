...
depletion.rate <- 0.5 # .25  to represent biennial fallow, .5 otherwise
```
...
We have capabilities for fallowing, but for simplicity let's turn that off by default
```{r land_req}
land.req <- function(population, yield, laborers, fallow = F){
...
create.households <- function(x){
  tibble(household = 1:x,
    ...
    mutate(inhabitants = map(n_inhabitants, create.inhabitants),
           laborers = map_dbl(inhabitants, ~filter(.x, age >= 20 & age < 45) %>% nrow),
           land = land.req(n_inhabitants, peak_end(yield_memory), laborers))
}
...
for(i in 1:nsim){
  this.yield <- overlay(soil, precip, fun = yield) %>% getValues() # *.5 for fallow
test <- test%>% 
  farm %>%
  eat %>%
  birth_death

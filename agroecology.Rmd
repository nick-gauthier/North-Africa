...
```{r eval = T}
library(parallel)
arable_proportion <- 1.0
sim.out <- tibble(year = 0, population = 0, fertility =  0, replicate = 0)
nsim <- 1000
replicates <- 50
precip <- 1.0

sim.out <- mclapply(1:replicates, function(j){
...
sim.out <- add_row(sim.out, year = i, population = sum(test$n_inhabitants), replicate = j)#, fertility = test$soil_fertility,)
}
return(sim.out[-1,])
}) %>% bind_rows

...
```{r}
land.req <- function(population, yield) calorie.req * population * (1 + 0) / yield
```
...
yield <- function(fertility, precipitation){
  ...
  return(max.yield * (f.reduction + p.reduction) / 2)
} 
...
eat <- function(households, harvest){
  households %>%
    mutate(total.cal.req = n_inhabitants * calorie.req,
           ...
           old.storage = storage,
           storage = if_else(total.cal.req <= storage, harvest, max(harvest - (total.cal.req - old.storage), 0))) %>%
    select(-old.storage, -total.cal.req)
}
```
...
test <- test%>% 
  mutate(land = land.req(n_inhabitants, peak_end(yield_memory))) %>% 
  mutate(harvest = land * this.yield) %>%
  eat %>%
  birth_death

...
```{r}
allocate_time <- function(l = 1, j = 0.2, r = 1, k = 0.6, psi = 0.2, e = 0.18, s = 1){
  #qmax <- s * r
  #region 2
...
tibble(laborers = seq(0,30,.1), 
       potential = max_cultivable_land(laborers, farming_labor, 100, type = 'unlimited'),
...
test <- settlement_locations %>%
  select(id,lon:lat) %>%
  rename(X = lon, Y = lat) %>%
SiteExploitationTerritories(.,elevation, slope = '', timeOfInterest = 4, numberOfIsochrones = 4) %>% 
  map(1)
test
rasterToContour(area(elevation),levels=seq(0, 2, 1))
```
...
prec_annual_2k <- prec * prec_200 / prec_2000

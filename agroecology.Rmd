...
food.req <- calorie.req / wheat.calories * wheat.cal.proportion # kg of wheat to feed a person for 1 year
...
land.req <- function(population, yield, laborers){
  land <- annual.wheat.req * population * (1 + seed_proportion) / yield
  pmin(land, max.ag.labor * laborers / labor.per.hectare) # constrain by maximum hectares per household 
}

bind_rows(tibble(inhabitants = 1:20, land = land.req(inhabitants, max.yield, 3), Management = 'No Fallow'),
          tibble(inhabitants = 1:20, land = 2 * land.req(inhabitants, max.yield, 3), Management = 'Fallow')) %>%
ggplot(aes(inhabitants, land, color = Management)) +
  geom_line() + 

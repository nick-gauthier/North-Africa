...
interact <- function(net){
  net %E>%
    mutate(interaction_strength = .N()$attractiveness[to] ^ alpha * exp(-beta * distance))  %>%
    group_by(from) %>%
    mutate(total_outflow = sum(interaction_strength)) %>%
    ungroup %>% 
    mutate(flow = .N()$population[from] * interaction_strength / total_outflow) %>%
    group_by(to) %>%
    mutate(inflow = sum(flow)) %>%
    ungroup %N>%
    mutate(inflow = .E()$inflow[name]) %>%

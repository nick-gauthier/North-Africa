---
title: "Modern Climate"
author: "Nick Gauthier"
date: "6/7/2017"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Present Day Climate in North Africa
```{r}
library(raster)
library(tidyverse)
library(ClimClass)
library(parallel)
```


```{r}
prec <- getData('worldclim', var = 'prec', res = 2.5) %>%
  crop(extent(-10, 15, 30, 40))

prec <- mask(prec, na.clump, maskvalue = 15, inverse = T)

tmin <- getData('worldclim', var = 'tmin', res = 2.5) %>%
  crop(extent(-10, 15, 30, 40)) %>%
  mask(na.clump, maskvalue = 15, inverse = T) %>%
  `/`(10)

tmax <- getData('worldclim', var = 'tmax', res = 2.5) %>%
  crop(extent(-10, 15, 30, 40)) %>%
  mask(na.clump, maskvalue = 15, inverse = T) %>%
  `/`(10)
```

```{r}
koeppen_map <- function(x){
  ifelse(is.na(prec[x][1]), 
         return(NA),
      return(data_frame(month = 1:12,
           P = c(prec[x]),
           Tx = c(tmax[x]),
           Tn = c(tmin[x])) %>%
    mutate(Tm = (Tx + Tn) / 2) %>%
    koeppen_geiger(clim.resume_verbose = F) %>%
    .$class %>%
      as.character))
}


clim_class <- mclapply(1:ncell(prec), koeppen_map, mc.cores = detectCores()-1) %>% 
  unlist %>%
  as.factor

setValues(prec[[1]], clim_class) %>% plot
```

